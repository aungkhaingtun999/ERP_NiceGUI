...
st.caption(
    "Select a request to view details and perform approval actions."
)
...
with st.container(
    ...
    with c2:
        ...
        st.write(
            f"**Warehouse:** "
            f"{warehouse_id or '❌ NOT PROVIDED'}"
        )
...
if is_own_request:
    ...
    # ----------------------------------------------------------------------
    # Reject can still be available to checker.
    # ----------------------------------------------------------------------
...
try:
    ...
    with st.container(
        ...
        if status == "APPROVED":
            ...
            st.success(
                f"✅ APPROVED — Request #{selected_history.get('id')}"
            )
            ...
            st.error(
                f"❌ REJECTED — Request #{selected_history.get('id')}"
            )
