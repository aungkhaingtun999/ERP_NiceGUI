...
def render_inventory_import():
    ...
    warehouse_name = st.selectbox(
        "Destination Warehouse", options=list(warehouse_options.keys())
    )

    warehouse_id = warehouse_options[warehouse_name]

    remarks = st.text_area(
        "Remarks", placeholder="Supplier invoice, opening stock, GRN note..."
    )
    ...
    with st.form("add_import_line_form", clear_on_submit=True):
        ...
        with c3:
            unit_cost = st.number_input(
                "Unit Cost *", min_value=0.0, value=0.0, step=100.0
            )
        ...
        with c5:
            mfg_date = st.date_input("MFG Date", value=None)
