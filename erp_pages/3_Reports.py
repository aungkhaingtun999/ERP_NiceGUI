...
def cashier_name(user):
    ...
    Current query provides: username employee_code
    """
...
def run():
    st.title("📊 ERP Executive Analytics & Reports")
    st.caption("MYANMAR ERP - Sales Performance Analytics")
    ...
    def get_sales(
        ...
        try:
            response = (
                ...
                .select(
                    """
                    id, total, total_amount, discount, tax, subtotal, paid_amount, payment_method, created_at, cashier_id, users(
            ...
            try:
                response = (
                    ...
                    .select(
                        """
                        id, total_amount, discount, tax, subtotal, paid_amount, payment_method, created_at, cashier_id, users(
                            username, employee_code
                        )
                        """
                    )
                ...
                st.error(f"Sales loading error: {retry_error}")
                return []
    ...
    if not sales:
        ...
        st.caption(f"Checked: {start_date} → {end_date}")
        return
    ...
    if "created_at" not in df.columns:
        st.error("Sales data does not contain created_at.")
        return
    ...
    if df.empty:
        st.warning("Sales records were found, but created_at values are invalid.")
        return
    ...
    cashier_options = ["All"] + sorted(cashier_values)
    ...
    if selected_cashier != "All":
        df = df[df["Cashier"] == selected_cashier].copy()
    ...
    # KPI
    # ==========================================================================
    revenue = safe_number(df["total"].sum())
    discount = safe_number(
        df["discount"].sum() if "discount" in df.columns else 0
    )
    tax = safe_number(
    ...
    with tab1:
        ...
        daily = (
            df.groupby(df["created_at"].dt.date)["total"]
            .sum()
            .reset_index()
        )
        daily.columns = [
